$(function(){
   ...
   function onFilterGroupClicked(group, allGroups){
      // Here we want to do a 'where in' filter.  'where groups in (list of active groups)'
      var activeGroups = [];
   ...
   visualizer = window.AVA.Visualizer({
      ...
      'dataCallback': filterGroups.onDataUpdated // Wire the data callback to the filter groups so it can be built
   });
